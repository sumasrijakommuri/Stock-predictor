...
$start_date = explode('-',date("m-d-y", mktime(0,0,0,4,13,2017)));
$end_date = explode('-',date("m-d-y"));

echo("start date:".$start_date[0]."/".$start_date[1]."/".$start_date[2]."</br>" );

$stock_ids = explode(",","GOOG,YHOO,MSFT,INTC,ERIC,NOK,CSCO,BAC,VZ,AAPL");
...
$get_data = "http://ichart.finance.yahoo.com/table.csv?s=".$id."&d=".$end_date[0]."&e=".$end_date[1].
	...
	$entries = file($file_name);
	 for($i = 0; $i< sizeof($entries); $i++)
	 {
		 ...
		 $insert_query = "insert into tendays(name,date,open,high,low,close,volume) values($attributes)";
